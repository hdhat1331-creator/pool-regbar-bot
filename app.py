...
# ========== لیست کاربران مسدود شده از دریافت پیام ==========
# هرکی توی این لیست باشه، هیچکس نمیتونه بهش پیام بده
BLOCKED_USERS = [
    # ایدی های عددی رو اینجا اضافه کن
    # مثال: 7408883098,
]

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DB_FILE = os.path.join(BASE_DIR, "pool_regbar_users.json")
MESSAGES_FILE = os.path.join(BASE_DIR, "pool_regbar_messages.json")
SETTINGS_FILE = os.path.join(BASE_DIR, "pool_regbar_settings.json")
TICKETS_FILE = os.path.join(BASE_DIR, "pool_regbar_tickets.json")
INVITES_FILE = os.path.join(BASE_DIR, "pool_regbar_invites.json")
GOLD_REQUESTS_FILE = os.path.join(BASE_DIR, "pool_regbar_gold_requests.json")
BANNED_FILE = os.path.join(BASE_DIR, "pool_regbar_banned.json")
BLOCKED_FILE = os.path.join(BASE_DIR, "pool_regbar_blocked.json")
...
def send_message(chat_id, text, reply_markup=None):
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ خطا در send_message: {response.text}")
            return False
    except Exception as e:
...
def send_message_to_user(chat_id, text):
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ خطا در send_message_to_user: {response.text}")
            return False
    except Exception as e:
...
def is_blocked(user_id):
    """بررسی اینکه کاربر در لیست مسدود شده از دریافت پیام هست یا نه"""
    blocked = load_db(BLOCKED_FILE)
...
def grant_gold_subscription(user_id, days):
    try:
        ...
        print(f"❌ خطا در grant_gold_subscription: {e}")
        return False
...
def get_user_stats():
    try:
        ...
        blocked = len(load_db(BLOCKED_FILE))
...
def reset_normal_users():
    try:
        ...
        print(f"❌ خطا در reset_normal_users: {e}")
        return False

def block_user(user_id):
    """مسدود کردن کاربر از دریافت پیام"""
    blocked = load_db(BLOCKED_FILE)
    blocked[str(user_id)] = True
    save_db(BLOCKED_FILE, blocked)
    return True

def unblock_user(user_id):
    """رفع مسدودیت کاربر از دریافت پیام"""
    blocked = load_db(BLOCKED_FILE)
...
def process_callback(callback):
    try:
        ...
        user_id = int(chat_id)
        data = callback["data"]
        print(f"📩 Callback دریافت شد: {data} از کاربر {user_id}")
        ...
        if data.startswith("copy_link_"):
            ...
            print(f"🔍 روزهای انتخاب شده: {days}")
            if days in GOLD_PLANS:
                plan = GOLD_PLANS[days]
                requests_db = load_db(GOLD_REQUESTS_FILE)
                if str(user_id) in requests_db:
                    send_message(chat_id, "❌ شما قبلاً درخواست اشتراک طلایی ثبت کرده‌اید!\nمنتظر پاسخ ادمین باشید.", main_keyboard())
                    return
                ...
                send_message(chat_id, f"✅ **درخواست شما ثبت شد!**\n📦 پلن: {plan['name']}\n💰 قیمت: {plan['price']:,} تومان\n⏳ مدت: {plan['days']} روز\n\n📞 برای تکمیل خرید، با ادمین تماس بگیرید:\n🆔 ادمین: `{ADMIN_ID}`\n\n⏰ پس از واریز، ادمین اشتراک شما را فعال می‌کند.", main_keyboard())
                send_message(ADMIN_ID, f"📩 **درخواست اشتراک طلایی جدید!**\n🆔 کاربر: `{user_id}`\n📦 پلن: {plan['name']}\n💰 قیمت: {plan['price']:,} تومان\n⏳ مدت: {plan['days']} روز\n🕐 زمان: {datetime.now().strftime('%Y-%m-%d %H:%M')}", get_gold_request_keyboard(user_id, plan['days']))
            else:
                send_message(chat_id, "❌ پلن نامعتبر!", main_keyboard())
            ...
            send_message(chat_id, "💬 پیام خود را برای کاربر بنویسید:", get_back_keyboard())
            ...
            if grant_gold_subscription(target_user, days):
                send_message(chat_id, f"✅ اشتراک طلایی {days} روزه به کاربر {target_user} اعطا شد!", get_admin_keyboard())
            else:
                send_message(chat_id, "❌ خطا در اعطای اشتراک!", get_admin_keyboard())
            ...
            if reply_text and ticket_id:
                ...
                if ticket_id in tickets:
                    ...
                    send_message(chat_id, "✅ پاسخ شما ارسال شد!", get_admin_keyboard())
            ...
            save_user_setting(ADMIN_ID, "replying_ticket", None)
            save_user_setting(ADMIN_ID, "state", None)
            send_message(chat_id, "❌ ارسال پاسخ لغو شد.", get_admin_keyboard())
            ...
            if not target_user:
                send_message(chat_id, "❌ خطا! کاربر مورد نظر یافت نشد.", get_admin_keyboard())
                return
            banned = load_db(BANNED_FILE)
            if days_str == "permanent":
                banned[str(target_user)] = "permanent"
                save_db(BANNED_FILE, banned)
                send_message(chat_id, f"🔒 کاربر {target_user} به طور دائمی مسدود شد!", get_admin_keyboard())
                send_message(target_user, "🚫 **شما توسط ادمین به طور دائمی مسدود شدید!**")
            else:
                days = int(days_str)
                banned[str(target_user)] = {"type": "temporary", "until": (datetime.now() + timedelta(days=days)).isoformat()}
                save_db(BANNED_FILE, banned)
                send_message(chat_id, f"🔒 کاربر {target_user} به مدت {days} روز مسدود شد!", get_admin_keyboard())
                send_message(target_user, f"🚫 **شما توسط ادمین به مدت {days} روز مسدود شدید!**")
            ...
            send_message(target_user, "🔒 **شما از دریافت پیام از طریق ربات مسدود شدید!**")
            ...
            if unblock_user(target_user):
                send_message(chat_id, f"✅ مسدودیت دریافت پیام کاربر {target_user} رفع شد!", get_admin_keyboard())
                send_message(target_user, "✅ **مسدودیت دریافت پیام شما رفع شد!**")
            else:
                send_message(chat_id, "❌ کاربر در لیست مسدود شده‌ها نیست!", get_admin_keyboard())
        
        elif data.startswith("set_msg_"):
            ref_by = data.replace("set_msg_", "")
            # بررسی اینکه کاربر مقصد در لیست مسدود شده از دریافت پیام هست یا نه
            if is_blocked(int(ref_by)):
                send_message(
                    chat_id,
                    f"❌ **ارسال پیام به این کاربر امکان پذیر نیست!**\n\n"
                    f"🆔 کاربر: `{ref_by}`\n"
                    f"این کاربر توسط ادمین مسدود شده است.",
                    main_keyboard()
                )
                return
            ...
            send_message(chat_id, "🔢 تعداد ارسال پیام را انتخاب کنید:", get_count_keyboard(ref_by))
            ...
            if not saved_message:
                send_message(chat_id, "❌ شما هنوز پیامی تنظیم نکرده‌اید!", main_keyboard())
                return
            lines = saved_message.count('\n') + 1
            user = get_user(user_id)
            is_gold = user and user.get("subscription_type") == "طلایی"
            if not is_gold and lines > 5:
                send_message(chat_id, "❌ بیش از ۵ خط! برای ارسال بیشتر نیاز به اشتراک طلایی دارید.", main_keyboard())
                return
            if not is_gold:
                invites = get_invites(user_id)
                if len(invites) >= 15:
                    send_message(chat_id, "❌ به حداکثر ۱۵ دعوت رسیده‌اید! برای دعوت بیشتر نیاز به اشتراک طلایی دارید.", main_keyboard())
                    return
            # بررسی اینکه کاربر مقصد در لیست مسدود شده از دریافت پیام هست یا نه
            ...
            if not is_admin(user_id):
                return
            reset_normal_users()
            send_message(chat_id, "✅ اطلاعات کاربران عادی با موفقیت ریست شد!", get_admin_keyboard())
        
        elif data == "reset_cancel":
            if not is_admin(user_id):
                return
            send_message(chat_id, "❌ عملیات ریست لغو شد.", get_admin_keyboard())
            ...
            if is_admin(user_id):
                send_message(chat_id, "👋 به پنل ادمین خوش آمدید!", get_admin_keyboard())
            else:
                user = get_user(user_id)
                if user:
                    send_message(chat_id, profile_text(user), main_keyboard())
            ...
            print(f"⚠️ Callback ناشناخته: {data}")
...
def process_message(msg):
    try:
        ...
        print(f"📩 پیام از {user_id}: {text[:50] if text else '(غیرمتنی)'}")
            ...
            if len(parts) == 3:
                try:
                    ...
                    if days in [7, 14, 30]:
                        ...
                        send_message(chat_id, "❌ مدت نامعتبر! فقط ۷، ۱۴ یا ۳۰ روز.", get_admin_keyboard())
                except:
        ...
        if text == "🏠 منو اصلی":
            ...
            if user:
                ...
                send_message(chat_id, "❌ لطفاً /start رو بزنید", main_keyboard())
            return
        ...
        if text and (text.startswith("/start") or text == "شروع"):
            ...
            if not user:
                ...
                if user:
                    ...
                    send_message(chat_id, "❌ خطا در ثبت‌نام! لطفاً دوباره تلاش کنید.", main_keyboard())
            else:
                if is_admin(user_id):
                    send_message(chat_id, "👋 به پنل ادمین خوش آمدید!", get_admin_keyboard())
                else:
        ...
        if is_admin(user_id):
            if text == "📊 آمار کاربران":
                ...
                send_message(chat_id, f"📊 **آمار کاربران:**\n\n👥 کل کاربران: {total}\n🟢 کاربران عادی: {normal}\n🟡 کاربران طلایی: {gold}\n🚫 مسدود شده‌ها: {banned}\n🔒 مسدود از دریافت پیام: {blocked}", get_admin_keyboard())
                return
            elif text == "🚫 مسدود کردن کاربر":
                send_message(chat_id, "🆔 آیدی عددی کاربر مورد نظر را ارسال کنید:", get_back_keyboard())
                ...
                if not pending:
                    send_message(chat_id, "✅ هیچ تیکت پاسخ‌داده‌نشده‌ای وجود ندارد.", get_admin_keyboard())
                    return
                for t in pending[:5]:
                    send_message(chat_id, f"📩 **تیکت جدید:**\n🆔 کاربر: `{t['user_id']}`\n📝 {t['message']}\n🕐 زمان: {t['time'][:16]}", get_ticket_keyboard(t['id']))
                return
            elif text == "📢 اطلاع‌رسانی":
                send_message(chat_id, "📝 پیام اطلاع‌رسانی خود را بنویسید:", get_back_keyboard())
                save_user_setting(ADMIN_ID, "state", "admin_broadcast")
                return
            elif text == "🔄 ریست اطلاعات کاربران عادی":
                send_message(chat_id, "⚠️ **آیا از ریست اطلاعات تمام کاربران عادی مطمئن هستید؟**\n\nاین کار باعث پاک شدن اطلاعات زیر می‌شود:\n• تعداد دعوت‌ها\n• موجودی (به ۵۰,۰۰۰ تومان برمی‌گردد)\n• لیست دعوت‌شده‌ها\n\nاین عملیات قابل بازگشت نیست!", get_reset_confirm_keyboard())
                return
                ...
                send_message(chat_id, "⚙️ **تنظیمات ربات:**\n\n💰 مبلغ جایزه دعوت: ۵۰,۰۰۰ تومان\n💎 قیمت اشتراک طلایی:\n   • ۷ روزه: ۵۰,۰۰۰ تومان\n   • ۱۴ روزه: ۱۰۰,۰۰۰ تومان\n   • یکماهه: ۱۲۰,۰۰۰ تومان\n📨 محدودیت پیام عادی: ۵۰ پیام\n📝 محدودیت خطوط: ۵ خط\n👥 محدودیت دعوت: ۱۵ نفر", get_admin_keyboard())
                return
            elif text == "🔙 بازگشت":
                save_user_setting(ADMIN_ID, "state", None)
                send_message(chat_id, "🔙 به پنل ادمین برگشتید.", get_admin_keyboard())
                return
        ...
        if admin_state == "admin_gold_message" and is_admin(user_id):
            ...
            if target_user:
                ...
                send_message(chat_id, f"✅ پیام شما به کاربر {target_user} ارسال شد!", get_admin_keyboard())
                save_user_setting(ADMIN_ID, "state", None)
        ...
        if admin_state == "admin_ticket_reply" and is_admin(user_id):
            ...
            send_message(chat_id, f"📝 **پاسخ شما:**\n\n{text}\n\nآیا از ارسال این پاسخ مطمئن هستید؟", get_reply_confirm_keyboard())
            return
        ...
        if admin_state == "admin_ban_user" and is_admin(user_id):
            if text.isdigit():
                ...
                if get_user(target_user):
                    ...
                    send_message(chat_id, f"🔒 **مدت زمان مسدودسازی برای کاربر {target_user} را انتخاب کنید:**", get_ban_days_keyboard())
                    save_user_setting(ADMIN_ID, "state", None)
                else:
                    send_message(chat_id, "❌ کاربر یافت نشد!", get_admin_keyboard())
            else:
                send_message(chat_id, "❌ لطفاً یک آیدی عددی معتبر ارسال کنید.", get_admin_keyboard())
            return
        ...
        if admin_state == "admin_grant_gold" and is_admin(user_id):
            if text.isdigit():
                ...
                if get_user(target_user):
                    ...
                    send_message(chat_id, f"🎁 **مدت اشتراک طلایی برای کاربر {target_user} را انتخاب کنید:**", get_admin_gold_plans_keyboard())
                    save_user_setting(ADMIN_ID, "state", None)
                else:
                    send_message(chat_id, "❌ کاربر یافت نشد!", get_admin_keyboard())
            else:
        ...
        if admin_state == "admin_broadcast" and is_admin(user_id):
            ...
            for uid in db.keys():
                if not is_banned(int(uid)):
                    ...
                    sent += 1
                time.sleep(0.05)
            send_message(chat_id, f"✅ پیام به {sent} کاربر ارسال شد.", get_admin_keyboard())
            save_user_setting(ADMIN_ID, "state", None)
            return
            ...
            if text.isdigit():
                ...
                if get_user(target_user):
                    if is_blocked(target_user):
                        send_message(chat_id, f"⚠️ کاربر {target_user} قبلاً مسدود شده است.\nآیا می‌خواهید مسدودیت را بردارید؟", get_unblock_confirm_keyboard(target_user))
                    else:
                        send_message(chat_id, f"⚠️ آیا از مسدود کردن دریافت پیام کاربر {target_user} مطمئن هستید؟", get_block_confirm_keyboard(target_user))
                    save_user_setting(ADMIN_ID, "state", None)
                else:
                    send_message(chat_id, "❌ کاربر یافت نشد!", get_admin_keyboard())
            else:
        ...
        if user_state == "waiting_for_invite_number":
            ...
            if text.isdigit():
                ...
                if 1 <= num <= len(real_invites):
                    ...
                    send_message(chat_id, "❌ عدد نامعتبر! لطفاً عددی از لیست انتخاب کنید.", main_keyboard())
            else:
                send_message(chat_id, "❌ لطفاً یک عدد معتبر ارسال کنید.", main_keyboard())
            return
        ...
        if user_state == "waiting_for_message" or user_state == "editing_message":
            ...
            save_user_setting(user_id, "state", None)
            send_message(chat_id, f"📝 **پیام شما ذخیره شد:**\n\n{text}\n\nآیا از تنظیم این پیام مطمئن هستید؟", get_confirm_keyboard())
            return
        ...
        if text == "📊 پروفایل":
            ...
            send_message(chat_id, "📝 **مشکل یا سوال خود را بنویسید:**", {"keyboard": [[{"text": "🔙 بازگشت"}], [{"text": "🏠 منو اصلی"}]], "resize_keyboard": True})
            ...
            save_db(TICKETS_FILE, tickets)
            send_message(chat_id, "✅ تیکت شما ثبت شد. به زودی پاسخ داده می‌شود.", main_keyboard())
