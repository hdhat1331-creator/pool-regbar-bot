...
BLOCKED_USERS = [
    # ایدی های مسدود شده از دریافت پیام
]
    ...
    try:
        ...
        if data.startswith("copy_link_"):
            ...
            if days in GOLD_PLANS:
                ...
                send_message(ADMIN_ID, f"📩 **درخواست اشتراک طلایی جدید!**\n🆔 کاربر: `{user_id}`\n📦 پلن: {plan['name']}\n💰 قیمت: {plan['price']:,} تومان\n⏳ مدت: {plan['days']} روز\n🕐 زمان: {datetime.now().strftime('%Y-%m-%d %H:%M')}", get_gold_request_request_keyboard(user_id, plan['days']))
            else:
            ...
            send_message(chat_id, f"✅ تعداد {count} پیام ذخیره شد.\n\n📌 پس از تایید و ارسال، پیام‌ها برای کاربر ارسال می‌شوند.", main_keyboard())
            time.sleep(0.3)
                ...
                try:
                    ...
                    for i in range(message_count):
                        ...
                        if not is_sender_gold:
                            ...
                            time.sleep(0.33)  # هر ثانیه ۳ پیام
