...
ADMIN_ID = 8235703809
PROTECTED_USERS = [ADMIN_ID]  # فقط ادمین توی لیسته، بقیه خودکار اضافه میشن
...
def is_protected(user_id):
    """بررسی اینکه کاربر در لیست ممنوع‌الپیام هست یا نه"""
...
def grant_gold_subscription(user_id, days):
    try:
        ...
        save_db(DB_FILE, db)
        # اضافه کردن به لیست ممنوع‌الپیام
...
def send_new_user_notification(ref_by, new_user_id):
    # بررسی کاربر مقصد (new_user_id) برای اشتراک طلایی
    target_user = get_user(new_user_id)
    if target_user and target_user.get("subscription_type") == "طلایی":
        send_message(
            ref_by,
            f"❌ **ارسال پیام به این کاربر امکان پذیر نیست!**\n\n"
            f"🆔 کاربر: `{new_user_id}`\n"
            f"این کاربر دارای اشتراک **طلایی** می‌باشد و ارسال پیام به او ممنوع است.",
            main_keyboard()
        )
        return
...
def process_callback(callback):
    try:
        ...
        if data.startswith("copy_link_"):
            ...
            # بررسی کاربر مقصد (ref_by) برای اشتراک طلایی
            target_user = get_user(int(ref_by))
            if target_user and target_user.get("subscription_type") == "طلایی":
                send_message(
                    chat_id,
                    f"❌ **ارسال پیام به این کاربر امکان پذیر نیست!**\n\n"
                    f"🆔 کاربر: `{ref_by}`\n"
                    f"این کاربر دارای اشتراک **طلایی** می‌باشد.",
                    main_keyboard()
                )
                return
            ...
            if not is_gold:
                ...
                if len(invites) >= 15:
                    ...
                    return
            # بررسی کاربر مقصد برای اشتراک طلایی
            target_user = get_user(new_user_id)
            if target_user and target_user.get("subscription_type") == "طلایی":
                send_message(
                    chat_id,
    ...
    try:
        ...
        if is_admin(user_id):
            if text == "📊 آمار کاربران":
                ...
                send_message(chat_id, f"📊 **آمار کاربران:**\n\n👥 کل کاربران: {total}\n🟢 کاربران عادی: {normal}\n🟡 کاربران طلایی: {gold}\n🚫 مسدود شده‌ها: {banned}", get_admin_keyboard())
                return
